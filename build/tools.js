 ...
 * 从OpenAPI路径操作中提取参数模式
 */
function extractParamsSchema(operation) {
    const properties = {};
    const required = [];
    // 处理路径参数
 ...
 */
export async function registerYCloudTools(server, openApiSpec, apiBaseUrl, headers) {
        ...
        ['get', 'post', 'put', 'delete', 'patch'].forEach(method => {
            ...
            if (operation) {
                ...
                if (!registeredTools.has(toolName)) {
                    server.tool(toolName, description, paramsSchema, async (args) => {
                        try {
                            // 解析URL中的路径参数
